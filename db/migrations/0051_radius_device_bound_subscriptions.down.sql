-- Reverse Task 3 device binding by restoring the migration 0047 definition of
-- radius_portal_handoff_authorize verbatim (MAC find-or-register under
-- max_devices, with no subscription.device_id enforcement). Forward-only policy:
-- this does not touch subscriptions.device_id (owned by migration 0050).
