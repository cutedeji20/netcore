-- Enforce each plan's device and concurrent-session limits at the RADIUS
-- authorization boundary.  A browser handoff remains a short-lived intent;
-- only this SECURITY DEFINER function may turn it into an Access-Accept.
...
CREATE TABLE radius_access_reservations (
    ...
    CONSTRAINT radius_access_reservations_expiry_bounded CHECK (
        expires_at > created_at
        AND expires_at <= created_at + interval '120 seconds'
    ),
    UNIQUE (subscription_id, normalized_mac)
);

CREATE INDEX radius_access_reservations_expiry_idx
    ON radius_access_reservations (expires_at);

CREATE FUNCTION radius_access_reservation_release() RETURNS trigger
LANGUAGE plpgsql
SET search_path = public, pg_temp
AS $$
BEGIN
    -- Accounting-Start is the durable proof that the NAS accepted the
    -- reservation.  A STOP-only historical record leaves it to its short TTL.
    IF NEW.status <> 'CLOSED' AND NEW.normalized_mac IS NOT NULL THEN
        DELETE FROM radius_access_reservations
         WHERE subscription_id = NEW.subscription_id
           AND normalized_mac = NEW.normalized_mac;
    END IF;
    RETURN NEW;
END;
$$;

CREATE TRIGGER sessions_release_radius_access_reservation
    AFTER INSERT ON sessions
    FOR EACH ROW EXECUTE FUNCTION radius_access_reservation_release();

ALTER TABLE radius_access_reservations ENABLE ROW LEVEL SECURITY;
ALTER TABLE radius_access_reservations FORCE ROW LEVEL SECURITY;
CREATE POLICY tenant_isolation ON radius_access_reservations
    USING (tenant_id = current_tenant_id())
    WITH CHECK (tenant_id = current_tenant_id());

-- The application never needs to read or write authorization reservations.
-- The RADIUS login role receives only the function below.
REVOKE ALL ON radius_access_reservations FROM PUBLIC, netcore_app_rw, netcore_radius;
