(function () {
  ...
  function request() {
    return fetch(apiBase + "/api/v1/integrations", { credentials: "include", cache: "no-store" })
      ...
      .then(function (body) { payload = body && Array.isArray(body.integrations) ? body.integrations : []; render(); })
      .catch(function () { payload = []; render(); });
  }
  ...
  function render() {
    ...
    window.NetCoreIntegrationDisplay.toCards(payload).forEach(function (card) {
