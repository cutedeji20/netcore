...
func TestPortalDeploymentTemplatesPreserveHandoffBoundaries(t *testing.T) {
	...
	if !strings.Contains(policy, "Mikrotik-Total-Limit-Gigawords") || !strings.Contains(policy, "^[A-Za-z0-9_-]{43}$") || !strings.Contains(policy, "concat_ws(',',") || !strings.Contains(policy, "Tmp-String-0 := \"%{control:Tmp-String-0}\"") || !strings.Contains(policy, "if (&Tmp-String-0 !~ /^([^,]+),([^,]+)") {
